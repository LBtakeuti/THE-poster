...
> **danger（赤）の使用ルール（keitakeuchi 判断 2026-06-15）**
> - 低彩度の赤 `#b3402b` を「危険/エラー」の意味色として **管理画面（/admin/*）にのみ** 使う。
> - 用途は限定：フォームのバリデーションエラー文言 / 必須マーク `*` / 破壊操作（削除）のリンク・枠。
> - **ストア（公開ページ）には赤を使わない。** 白基調・ミニマルを厳守（試作HTML準拠）。
> - 多用しない。意味（危険・エラー）がある箇所だけ。装飾目的では使わない。
